...
void GameShop::RequestItemBuy(int aIndex, GAMESHOP_REQ_BUY * lpRequest)
{
	...
	LogAddC(3, "[DEBUG] [%s] %d %d %d [%d %d]", __FUNCTION__, 
		lpRequest->ItemID, lpRequest->IBSCategory
		, lpRequest->IBSIndex, lpRequest->IBSPackageID, lpRequest->Unknown18);
#endif
	// ----
	LPOBJ lpUser = &gObj[aIndex];
	// ----
...
void GameShop::RequestStorage(int aIndex, GAMESHOP_REQ_STORAGE * lpRequest)
{
	...
	for( int i = 0; i < MAX_STORAGE_ITEM; i++ )
	{
		...
		else if( lpRequest->StorageType == 0x47 && lpUser->GameShop.GiftStorage[i].AuthIndex != 0 && lpUser->GameShop.GiftStorage[i].AuthID != 0 )//start test
		{
...
void GameShop::RequestStorageItemUse(int aIndex, GAMESHOP_REQ_STORAGEITEMUSE * lpRequest)
{
	...
	else if(lpItem->ItemID == ITEMGET(14, 91))		//Fix Summoner and RageFighter card
	{
		GDAccountInfoStateUpdate(lpUser, lpUser->m_Index, 0);
		GCServerMsgStringSend("You can create Summoner character", aIndex, 1);
	}
	else if(lpItem->ItemID == ITEMGET(14, 169))		//Fix Summoner and RageFighter card
	{
		GDAccountInfoStateUpdate(lpUser, lpUser->m_Index, 1);
...
int GameShop::GetItemCountInStorage(int aIndex, int ClientStorageType)	//-> Complete
{
	...
	for( int i = 0; i < MAX_STORAGE_ITEM; i++ )
	{
		...
		{
			if( lpUser->GameShop.GiftStorage[i].AuthIndex != 0 && lpUser->GameShop.GiftStorage[i].AuthID != 0 )//start test
			{
				Count++;
			}//end test
		}
...
bool GameShop::DeleteItemInStorage(int aIndex, int AuthIndex, int AuthID)	//-> Complete
{
	...
	for( int i = 0; i < MAX_STORAGE_ITEM; i++ )
	{
		...
		else if( lpUser->GameShop.GiftStorage[i].AuthIndex == AuthIndex && lpUser->GameShop.GiftStorage[i].AuthID == AuthID )//start test
		{
