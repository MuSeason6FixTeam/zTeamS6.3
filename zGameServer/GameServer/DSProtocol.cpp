...
void DataServerProtocolCore(BYTE protoNum, BYTE *aRecv, int aLen)
{
	switch ( protoNum )
	{
		...
		//Season3
		case 0xA1:	//Fix Summoner and RageFighter card
			DGAccountInfoStateUpdatedSend((SDHP_ANS_ACCOUNTINFO *)aRecv);
			break;
		case 0xA2:	//Fix Summoner and RageFighter card
...
typedef struct
{
	...
	short aIndex;
	BYTE isRageFighter;
}PMSG_REQ_SETACCOUNTINFO_CREATE, *LPPMSG_REQ_SETACCOUNTINFO_CREATE;


//Identical (CashShop Buy)
void GDAccountInfoStateUpdate(LPOBJ lpObj, int iIndex, BYTE isRageFighter)
{
	PMSG_REQ_SETACCOUNTINFO_CREATE pMsg;
	...
	pMsg.isRageFighter = isRageFighter;
